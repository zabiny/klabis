...
2. **Adapters live in the integration's own module** and implement the `sync` module's `SynchronizationAdapter` secondary port. `OrisEventSyncAdapter` (`com.klabis.oris.eventsync`) reaches `events` only through its `events.application` primary ports (`EventManagementPort`, `OrisEventFieldsGateway`) — the same direction `OrisController` already took — so `events` gains no new knowledge of ORIS internals.
