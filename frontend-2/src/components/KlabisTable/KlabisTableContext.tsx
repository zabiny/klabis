...
import type {PaginatedApiParams, SortDirection} from '../../api'
...
class ColumnModel {
    ...
    renderCellForRow(item: Record<string, unknown>): ReactNode {
        ...
        return (
            <TableDataCell key={this.key || this.name}>
                {cellContent}
            </TableDataCell>
        )
    }
...
const convertToColumnModel = (child: ReactNode): ColumnModel | null => {
    if (isTableCellComponent(child)) {
        return new ColumnModel(
            child.props.column,
            child,
            child.props.hidden || false,
            child.props.sortable || false,
            child.key || undefined,
            child.props.dataRender || defaultRenderFunc
        )
    }
