...
- [ ] 1.1 Vytvořit `TrainerLevel` enum v `com.klabis.members.domain` s hodnotami `T1`, `T2`, `T3`
  - Soubor: `backend/src/main/java/com/klabis/members/domain/TrainerLevel.java`
- [ ] 1.2 Přidat field `level` (typ `TrainerLevel`, required) do `TrainerLicense` value objectu
...
- [ ] 1.3 Napsat unit testy pro `TrainerLicense` s novým `level` fieldem
  - Soubor: `backend/src/test/java/com/klabis/members/domain/TrainerLicenseTest.java`

## 2. Domain — RefereeLicense value object

- [ ] 2.1 Vytvořit `RefereeLevel` enum v `com.klabis.members.domain` s hodnotami `R1`, `R2`, `R3`
  - Soubor: `backend/src/main/java/com/klabis/members/domain/RefereeLevel.java`
- [ ] 2.2 Vytvořit `RefereeLicense` value object analogicky ke `TrainerLicense`
...
- [ ] 2.3 Napsat unit testy pro `RefereeLicense`
  - Soubor: `backend/src/test/java/com/klabis/members/domain/RefereeLicenseTest.java`

## 3. Domain — Member aggregate

- [ ] 3.1 Přidat field `refereeLicense` (typ `RefereeLicense`, nullable) do `Member` aggregate
  - Soubor: `backend/src/main/java/com/klabis/members/domain/Member.java`
- [ ] 3.2 Přidat `refereeLicense` do `Member.SelfUpdate` command recordu (admin-only pole — viz poznámka)
- [ ] 3.3 Přidat `refereeLicense` do `Member.UpdateMemberByAdmin` command recordu
- [ ] 3.4 Přidat `refereeLicense` zpracování do `handle(SelfUpdate)` a `handle(UpdateMemberByAdmin)` metod
- [ ] 3.5 Přidat `getRefereeLicense()` getter
- [ ] 3.6 Aktualizovat `register()` factory metodu a `reconstruct()` factory metodu (přidat `refereeLicense` parametr)
...
- [ ] 4.1 Přidat sloupce do migration skriptu `V001__initial_schema.sql` v tabulce `members`:
...
- [ ] 4.2 Aktualizovat `MemberMemento`:
...
- [ ] 4.3 Napsat nebo aktualizovat repository integration test
...
- [ ] 5.1 Aktualizovat `TrainerLicenseDto`:
...
- [ ] 5.2 Vytvořit `RefereeLicenseDto`:
...
- [ ] 5.6 Aktualizovat `UpdateMemberRequestMapper`:
...
- [ ] 5.7 Napsat nebo aktualizovat controller integration test pro `GET /api/members/{id}`:
...
- [ ] 6.1 Vygenerovat nové API typy ze Swagger schématu
  - Příkaz: `npm run generate-api` (nebo ekvivalent dle `frontend/CLAUDE.md`)
  - Soubor: `frontend/src/api/klabisApi.d.ts`
- [ ] 6.2 Aktualizovat `MemberDetailPage.tsx` — sekce "Doklady a licence":
...
- [ ] 6.3 Aktualizovat `KlabisFieldsFactory.tsx` — form pole pro edit mód:
...
- [ ] 7.1 Spustit backend testy přes `test-runner` agenta
- [ ] 7.2 Spustit frontend testy: `npm run test`
- [ ] 7.3 Ověřit manuálně v prohlížeči: přihlásit se jako admin, otevřít detail člena, zkontrolovat sekci "Doklady a licence"
