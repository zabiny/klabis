...
- [x] 3.4 Frontend: add a route (e.g. `/events/:id/accommodation-list`) that fetches the endpoint and renders a print-friendly table; CSS `@media print` for legible printout; "Tisknout" button calls `window.print()`; render "neuvedeno" in cells where backend returned `null`
- [x] 3.5 Frontend: in event detail action bar, render "Seznam pro ubytování" action only when the `accommodation-list` link is present in the response; clicking navigates to the new route
