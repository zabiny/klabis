...
- [x] 6.5 Ruční QA v prohlížeči na `http://localhost:3000`: vytvoření eventu s kategoriemi a cenou (✓), detail s pills (✓), přejmenování kategorie se zachováním registrací a jejich id (✓), přidání/odebrání kategorie (✓). QA odhalilo blokující bug v editoru kategorií (array field hijack) — opraveno v commitu 9caeb610. Sync z ORIS neověřen v UI (sdílená dev data), pokryto E2E testy backendu.
