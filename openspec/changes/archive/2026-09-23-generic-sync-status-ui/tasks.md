...
- [x] 9.6 Fix `/api/api/` duplicate prefix in submit URL. After 9.3 the form modal opens with the override, but POST `/api/api/EVENTS/.../sync` 404'd: `HalFormButton` snapshots `useHalPageData().route.pathname`, which inside a sync `HalRouteProvider` is the self-link `/api/EVENTS/.../sync` (resource-shaped absolute URL preserved by `useHalRoutePath`), not a plain router pathname; `HalFormDisplay.handleSubmit` then built `'/api' + pathname`, doubling the prefix. Three changes: import `normalizeKlabisApiPath` in `HalFormDisplay.tsx` and wrap the pathname fallback with it; pass `resourceUrl={effectiveResourceUrl}` from `HalFormsPageLayout.tsx` into `HalFormDisplay` (the layout already had it for `HalFormPanel`, the prop was never forwarded to the modal sibling); and apply the same normalization to `HalFormsPageLayout`'s `effectiveResourceUrl ?? '/api' + effectivePathname` collectionURL.
- [x] 9.7 Tests + revalidation — 2 new `HalFormDisplay.test.tsx` cases (synchronizeNow fallback hits `resourceUrl` → no duplicate; sync-only `pathname` fallback → normalize strips leading `/api`). Final: `npm run lint` 0 errors, `HalFormDisplay.test.tsx` 33/33 passes, full sync `HalFormDisplay` + `HalFormsPageLayout` + `SyncStatusOverlay` + `SyncStatusIndicator` + `EventsPage` 125/125 passes, `npm run build` OK. Live end-to-end could not be re-exercised because the local example-data profile no longer seeds `ZBM9000` with `oris` active; outcome confirmed via unit tests covering both submit URL paths.
