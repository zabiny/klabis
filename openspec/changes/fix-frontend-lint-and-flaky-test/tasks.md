...
- [ ] 2.1 Inventory the 271 `no-explicit-any` occurrences grouped by file/area (D3)
- [ ] 2.2 Batch 1: fix `any` in the first file group with accurate types — generated API types / generics / `unknown`+narrowing, no `eslint-disable`, no `as any` relocation (D1); gate with `tsc` build + affected tests + eslint; commit
- [ ] 2.3 Batch 2: next file group, same rules and gate (D1, D3); commit
