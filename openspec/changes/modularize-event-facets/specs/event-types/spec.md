...
The system SHALL provide a catalog of event types that can be assigned to club events. Each event type has a unique name (case-insensitive), an optional display color (hex), a sort order that determines its position in lists and filters, and a set of **default facets** that are pre-filled on an event when the type is selected during creation. Users with the EVENTS:MANAGE authority SHALL be able to create, rename, recolor, reorder, set the default facets of, and delete event types.
...
#### Scenario: Manager sets the default facets of an event type

- **GIVEN** an event type "Pohárový závod" exists
- **WHEN** the manager sets its default facets to Categories, Ranking, and Pricing
- **THEN** creating a new event with type "Pohárový závod" pre-fills the Categories, Ranking, and Pricing facets

#### Scenario: Default facets of a training type

- **GIVEN** an event type "Trénink" exists
- **WHEN** the manager sets its default facets to Categories only
- **THEN** creating a new event with type "Trénink" pre-fills only the Categories facet
