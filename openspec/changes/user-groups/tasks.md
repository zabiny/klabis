...
- [ ] 4.1 Event listener: `MemberCreatedListener` — listen to `MemberCreatedEvent`, auto-assign to matching training group by age
- [ ] 4.2 Scheduled task: `AgeBasedReassignmentScheduler` — periodic check of all training group members, reassign/remove members whose age no longer matches
- [ ] 4.3 Domain events: publish events on member reassignment (for future notifications)
- [ ] 4.4 Backend: add training group info to member detail response (group name, owner name + contact)
- [ ] 4.5 Frontend: training group section on member profile page (group name, owner contact info; hidden if not assigned)
- [ ] 4.6 Tests: listener integration test (MemberCreatedEvent → auto-assignment), scheduler test (age-out scenarios), member detail response test
