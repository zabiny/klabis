...
- [ ] 9.1 Add `DisciplineManagementPort`/`DisciplineManagementService` (`events.application`) with `create`, `update`, `archive`, `restore`, `list`, `get`, mirroring `EventTypeManagementPort`/`Service`; write unit tests for each operation including the "archive always succeeds even if referenced" and "restore fails with 409-equivalent if not archived" cases
- [ ] 9.2 Add the `Discipline` resource to `docs/openapi/spec/events.yaml` (`GET`/`POST /api/disciplines`, `GET`/`PUT`/`DELETE /api/disciplines/{id}`, `POST /api/disciplines/{id}/restore`) with `x-klabis-authority` (`EVENTS_READ`/`EVENTS_MANAGE`), `x-hal-links` and `x-hal-templates` per design.md's API Changes table; regenerate the API interfaces and verify the build compiles
- [ ] 9.3 Implement `DisciplineController` implementing the generated `DisciplinesApi`, with HAL links (`self`, `collection`) and conditional affordances (`createDiscipline`, `updateDiscipline`, exactly one of `archiveDiscipline`/`restoreDiscipline`) gated by authority and archived state; write a `@WebMvcTest` verifying the HAL+FORMS shape for both an active and an archived discipline
