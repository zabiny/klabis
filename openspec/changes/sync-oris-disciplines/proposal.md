...
### New Capabilities
- `disciplines`: the local, ORIS-sourced catalog of orienteering disciplines that event types can be mapped to, kept in step automatically.
...
- **API**: `EventTypeManagementService.listDisciplineOptions()` (backing the discipline picklist affordance) now reads local data; no request/response shape change for existing consumers.
