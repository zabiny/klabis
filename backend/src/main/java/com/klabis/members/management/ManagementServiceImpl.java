...
class ManagementServiceImpl implements ManagementService {
        ...
        if (!isAdmin) {
            if (!canSelfEdit(existingMember, authUserRegistrationId)) {
                if (authUserRegistrationId != null) {
                    ...
                    throw new IllegalStateException("Cannot update member without authenticated user");
                }
            }
    ...
    @Transactional
    @Override
    public UUID terminateMember(UUID memberId, Member.TerminateMembership command) {
        // Get authentication context
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        if (authentication == null || !authentication.isAuthenticated()) {
        ...
        log.info("Membership terminated: memberId={}, terminatedAt={}, terminatedBy={}",
                savedMember.getId(), savedMember.getDeactivatedAt(), command.terminatedBy());
