...
-- 1. members.members (no dependencies)
...
-- Indexes for members
CREATE INDEX idx_members_registration_number ON members.members (registration_number);
CREATE INDEX idx_members_last_name ON members.members (last_name);
CREATE INDEX idx_members_date_of_birth ON members.members (date_of_birth);
CREATE INDEX idx_members_is_active ON members.members (is_active);

-- Comments for members table
COMMENT ON TABLE members.members IS 'Stores club member registration and personal information';
COMMENT ON COLUMN members.members.registration_number IS 'Unique registration number in format XXXYYDD (club code + birth year + sequence)';
COMMENT ON COLUMN members.members.email IS 'Member primary email address (validated at application layer)';
COMMENT ON COLUMN members.members.phone IS 'Member primary phone number (validated at application layer)';
COMMENT ON COLUMN members.members.street IS 'Street address (nullable)';
COMMENT ON COLUMN members.members.city IS 'City name (nullable)';
COMMENT ON COLUMN members.members.postal_code IS 'Postal or ZIP code (nullable)';
COMMENT ON COLUMN members.members.country IS 'Country code using ISO 3166-1 alpha-2 format (nullable, e.g., CZ, US, SK)';
COMMENT ON COLUMN members.members.is_active IS 'Soft delete flag - false means member is deactivated';
COMMENT ON COLUMN members.members.chip_number IS 'Member identification chip number (nullable, max 50 characters)';
COMMENT ON COLUMN members.members.identity_card_number IS 'Identity card number (nullable, max 50 characters)';
COMMENT ON COLUMN members.members.identity_card_validity_date IS 'Identity card validity expiration date (nullable)';
COMMENT ON COLUMN members.members.medical_course_completion_date IS 'Medical course completion date (nullable)';
COMMENT ON COLUMN members.members.medical_course_validity_date IS 'Medical course validity expiration date (nullable)';
COMMENT ON COLUMN members.members.trainer_license_level IS 'Trainer license level (nullable, enum T1/T2/T3)';
COMMENT ON COLUMN members.members.trainer_license_validity_date IS 'Trainer license validity expiration date (nullable)';
COMMENT ON COLUMN members.members.referee_license_level IS 'Referee license level (nullable, enum R1/R2/R3)';
...
-- Indexes for users
CREATE INDEX idx_users_user_name ON common.users (user_name);
CREATE INDEX idx_users_account_status ON common.users (account_status);
CREATE INDEX idx_users_created_at ON common.users (created_at);

-- Comments for users table
COMMENT ON TABLE common.users IS 'User accounts for authentication';
COMMENT ON COLUMN common.users.user_name IS 'Unique username for login. For members, auto-populated from registration number (XXXYYDD format). For standalone users, can be any unique string.';
COMMENT ON COLUMN common.users.password_hash IS 'BCrypt-hashed password';
...
CREATE INDEX idx_user_permissions_user_id ON common.user_permissions (user_id);

-- Comments for user_permissions
COMMENT ON TABLE common.user_permissions IS 'User permissions/authorities, separated from User entity for authorization';
COMMENT ON COLUMN common.user_permissions.user_id IS 'Reference to user (UUID)';
COMMENT ON COLUMN common.user_permissions.authorities IS 'Direct authorities as JSON array string (e.g., ["MEMBERS:READ", "TRAINING:VIEW"])';
COMMENT ON COLUMN common.user_permissions.created_at IS 'Timestamp when permissions were created';
COMMENT ON COLUMN common.user_permissions.created_by IS 'User who created these permissions';
COMMENT ON COLUMN common.user_permissions.modified_at IS 'Timestamp when permissions were last modified';
COMMENT ON COLUMN common.user_permissions.last_modified_by IS 'User who last modified these permissions';
...
CREATE TABLE common.password_setup_tokens
(
    id         UUID PRIMARY KEY,
...
CREATE INDEX idx_password_tokens_user_id ON common.password_setup_tokens (user_id);
CREATE INDEX idx_password_tokens_token_hash ON common.password_setup_tokens (token_hash);
CREATE INDEX idx_password_tokens_expires_at ON common.password_setup_tokens (expires_at);
CREATE INDEX idx_password_tokens_created_at ON common.password_setup_tokens (created_at);

-- Comments for password_setup_tokens
COMMENT ON TABLE common.password_setup_tokens IS 'Tokens for user self-service password setup during account activation';
COMMENT ON COLUMN common.password_setup_tokens.id IS 'Unique token identifier (UUID)';
COMMENT ON COLUMN common.password_setup_tokens.user_id IS 'Reference to user account requiring password setup';
COMMENT ON COLUMN common.password_setup_tokens.token_hash IS 'SHA-256 hash of the random token (never store plain text)';
COMMENT ON COLUMN common.password_setup_tokens.created_at IS 'Token generation timestamp';
COMMENT ON COLUMN common.password_setup_tokens.expires_at IS 'Token expiration timestamp (typically 4 hours after generation)';
COMMENT ON COLUMN common.password_setup_tokens.used_at IS 'Timestamp when token was used to set password (null if unused)';
COMMENT ON COLUMN common.password_setup_tokens.used_by_ip IS 'IP address of user who set password (for security audit)';
COMMENT ON COLUMN common.password_setup_tokens.created_by IS 'Identifier of who created this token (for audit trail)';
COMMENT ON COLUMN common.password_setup_tokens.modified_at IS 'Timestamp when token was last modified';
COMMENT ON COLUMN common.password_setup_tokens.modified_by IS 'Identifier of who last modified this token (for audit trail)';
...
CREATE UNIQUE INDEX idx_event_types_name ON events.event_types (name);
CREATE INDEX idx_event_types_sort_order ON events.event_types (sort_order);

-- Comments for event_types
COMMENT ON TABLE events.event_types IS 'Catalog of event types managed by admin (e.g. Training, Race, Championship)';
COMMENT ON COLUMN events.event_types.name IS 'Display name of the event type — unique case-insensitively';
COMMENT ON COLUMN events.event_types.color IS 'Optional hex color code for display (e.g. #ff0000)';
...
(
    ...
    event_type_id        UUID         NULL REFERENCES events.event_types(id),
...
CREATE INDEX idx_events_organizer ON events.events (organizer);

-- Comments for events
COMMENT ON TABLE events.events IS 'Stores orienteering event information';
COMMENT ON COLUMN events.events.id IS 'Unique event identifier (UUID)';
COMMENT ON COLUMN events.events.name IS 'Name of the event';
COMMENT ON COLUMN events.events.event_date IS 'Date when the event takes place';
...
COMMENT ON COLUMN events.events.modified_by IS 'User who last modified the event';
COMMENT ON COLUMN events.events.version IS 'Optimistic locking version';
...
CREATE INDEX idx_event_registrations_event_id ON events.event_registrations (event_id);

-- Comments for event_registrations
COMMENT ON TABLE events.event_registrations IS 'Stores member registrations for events';
COMMENT ON COLUMN events.event_registrations.id IS 'Unique registration identifier (UUID)';
...
CREATE TABLE calendar.calendar_items
(
    id               UUID         PRIMARY KEY,
...
CREATE INDEX idx_calendar_items_date_range ON calendar.calendar_items (start_date, end_date);
CREATE INDEX idx_calendar_items_event_id ON calendar.calendar_items (event_id);

-- Comments for calendar_items
COMMENT ON TABLE calendar.calendar_items IS 'Stores calendar items (manual and event-linked)';
COMMENT ON COLUMN calendar.calendar_items.id IS 'Unique calendar item identifier (UUID)';
COMMENT ON COLUMN calendar.calendar_items.kind IS 'Discriminator: MANUAL or EVENT_DATE (single-table inheritance)';
COMMENT ON COLUMN calendar.calendar_items.name IS 'Calendar item name (event name for linked items)';
COMMENT ON COLUMN calendar.calendar_items.description IS 'Calendar item description (location + organizer + website for linked items)';
COMMENT ON COLUMN calendar.calendar_items.start_date IS 'Start date of the calendar item';
COMMENT ON COLUMN calendar.calendar_items.end_date IS 'End date of the calendar item (same as start_date for single-day items)';
COMMENT ON COLUMN calendar.calendar_items.event_id IS 'Reference to linked event (NULL for manual items, ON DELETE SET NULL)';
COMMENT ON COLUMN calendar.calendar_items.created_at IS 'Timestamp when calendar item was created';
COMMENT ON COLUMN calendar.calendar_items.created_by IS 'User who created the calendar item';
COMMENT ON COLUMN calendar.calendar_items.modified_at IS 'Timestamp when calendar item was last modified';
COMMENT ON COLUMN calendar.calendar_items.last_modified_by IS 'User who last modified the calendar item';
COMMENT ON COLUMN calendar.calendar_items.version IS 'Optimistic locking version';
...
CREATE TABLE members.birth_number_audit_log
(
    id          UUID PRIMARY KEY,
...
CREATE INDEX idx_bn_audit_member_id ON members.birth_number_audit_log (member_id);
CREATE INDEX idx_bn_audit_user_id ON members.birth_number_audit_log (user_id);
CREATE INDEX idx_bn_audit_occurred_at ON members.birth_number_audit_log (occurred_at);

-- Comments for birth_number_audit_log
COMMENT ON TABLE members.birth_number_audit_log IS 'GDPR-compliant audit trail for birth number (rodné číslo) access and modification';
COMMENT ON COLUMN members.birth_number_audit_log.user_id IS 'ID of the user who accessed or modified the birth number';
COMMENT ON COLUMN members.birth_number_audit_log.member_id IS 'ID of the member whose birth number was accessed or modified (no FK – audit records are retained after member deletion)';
COMMENT ON COLUMN members.birth_number_audit_log.action IS 'Action type: VIEW_BIRTH_NUMBER or MODIFY_BIRTH_NUMBER';
...
CREATE INDEX idx_user_groups_type ON groups.user_groups (type);
CREATE INDEX idx_user_groups_name ON groups.user_groups (name);
...
CREATE TABLE groups.user_group_members
(
    user_group_id UUID      NOT NULL REFERENCES groups.user_groups (id) ON DELETE CASCADE,
    member_id     UUID      NOT NULL,
...
CREATE INDEX idx_user_group_members_group_id ON groups.user_group_members (user_group_id);
CREATE INDEX idx_user_group_members_member_id ON groups.user_group_members (member_id);

-- Comments for user_group_members
COMMENT ON TABLE groups.user_group_members IS 'Members of any group type with join timestamp';
COMMENT ON COLUMN groups.user_group_members.joined_at IS 'Timestamp when the member was added to the group';
...
CREATE TABLE groups.user_group_invitations
(
    id                   UUID        NOT NULL PRIMARY KEY,
    user_group_id        UUID        NOT NULL REFERENCES groups.user_groups (id) ON DELETE CASCADE,
...
CREATE INDEX idx_user_group_invitations_group_id ON groups.user_group_invitations (user_group_id);
CREATE INDEX idx_user_group_invitations_invited_member_status ON groups.user_group_invitations (invited_member_id, status);

-- Comments for user_group_invitations
COMMENT ON TABLE groups.user_group_invitations IS 'Membership invitations for FREE (FreeGroup) groups';
COMMENT ON COLUMN groups.user_group_invitations.status IS 'Invitation status: PENDING, ACCEPTED, REJECTED, CANCELLED';
COMMENT ON COLUMN groups.user_group_invitations.cancelled_at IS 'Timestamp when the invitation was cancelled';
COMMENT ON COLUMN groups.user_group_invitations.cancelled_by IS 'MemberId of the member who cancelled the invitation';
COMMENT ON COLUMN groups.user_group_invitations.cancellation_reason IS 'Optional free-text reason for cancellation';
...
CREATE TABLE events.category_presets
(
    id          UUID         NOT NULL PRIMARY KEY,
    name        VARCHAR(200) NOT NULL,
    categories  VARCHAR(2000) NULL,
...
CREATE INDEX idx_category_presets_name ON events.category_presets (name);

-- Comments for category_presets
COMMENT ON TABLE events.category_presets IS 'Reusable category preset templates for events (copy-on-apply, no link maintained)';
COMMENT ON COLUMN events.category_presets.categories IS 'Comma-separated category names (e.g. "M21,W35,D10"; null means empty preset)';
...
CREATE TABLE calendar.calendar_feed_token
(
    user_id     UUID         NOT NULL PRIMARY KEY REFERENCES common.users (id) ON DELETE CASCADE,
...
CREATE INDEX idx_calendar_feed_token_lookup ON calendar.calendar_feed_token (token_lookup);

-- Comments for calendar_feed_token
COMMENT ON TABLE calendar.calendar_feed_token IS 'Per-user PAT for iCalendar feed. One row per user (upsert on regenerate). Owned by the calendar module.';
COMMENT ON COLUMN calendar.calendar_feed_token.user_id IS 'Owner user ID — primary key (single token per user)';
COMMENT ON COLUMN calendar.calendar_feed_token.token_hash IS 'BCrypt/Argon2 hash of the raw token (never stored plain)';
COMMENT ON COLUMN calendar.calendar_feed_token.token_lookup IS 'Non-secret 8-char prefix of the raw base64url token, used for indexed pre-filter before bcrypt comparison';
COMMENT ON COLUMN calendar.calendar_feed_token.last_set_at IS 'When the token was last generated or regenerated';
...
CREATE TABLE finance.member_account
(
...
COMMENT ON TABLE finance.member_account IS 'Financial prepaid account per club member (1:1). Owned by the finance module.';
COMMENT ON COLUMN finance.member_account.member_id IS 'Primary key — equals members.id (shared identity, no FK constraint across module boundary)';
COMMENT ON COLUMN finance.member_account.balance_amount IS 'Current cached balance (sum of all transaction amounts); positive = credit, negative = debt';
COMMENT ON COLUMN finance.member_account.balance_currency IS 'ISO 4217 currency code (v1: CZK only)';
...
CREATE TABLE finance.finance_transaction
(
    id                      UUID           NOT NULL PRIMARY KEY,
    member_account_id       UUID           NOT NULL REFERENCES finance.member_account (member_id),
    type                    VARCHAR(20)    NOT NULL,
...
CREATE INDEX idx_finance_transaction_account_occurred ON finance.finance_transaction (member_account_id, occurred_at DESC);
...
CREATE UNIQUE INDEX idx_finance_transaction_reverses_unique ON finance.finance_transaction (reverses_transaction_id);

COMMENT ON TABLE finance.finance_transaction IS 'Append-only ledger of financial transactions per member account. Owned by the finance module.';
COMMENT ON COLUMN finance.finance_transaction.type IS 'Transaction type: DEPOSIT (positive, credit) or OTHER (negative, debit)';
COMMENT ON COLUMN finance.finance_transaction.amount IS 'Signed amount: DEPOSIT > 0, OTHER < 0';
COMMENT ON COLUMN finance.finance_transaction.recorded_at IS 'Server timestamp when the entry was recorded (audit, immutable)';
COMMENT ON COLUMN finance.finance_transaction.occurred_at IS 'Date when the underlying financial event occurred (entered by finance manager)';
...
CREATE INDEX idx_membership_fee_group_year ON membershipfees.membership_fee_group (group_year);
CREATE INDEX idx_membership_fee_group_source_level ON membershipfees.membership_fee_group (source_level_id);

-- Comments for membership_fee_group
COMMENT ON TABLE membershipfees.membership_fee_group IS 'Snapshot of a fee level for a specific year; contains membership records for member choices';
COMMENT ON COLUMN membershipfees.membership_fee_group.source_level_id IS 'Template level this snapshot was created from (for D8: previous-year default lookup)';
COMMENT ON COLUMN membershipfees.membership_fee_group.name IS 'Snapshot of the level name at publication time';
COMMENT ON COLUMN membershipfees.membership_fee_group.group_year IS 'Calendar year this group applies to';
COMMENT ON COLUMN membershipfees.membership_fee_group.yearly_fee_snapshot_amount IS 'Snapshot of the yearly fee at publication time; immutable after FROZEN';
COMMENT ON COLUMN membershipfees.membership_fee_group.status IS 'EDITABLE: snapshot can be changed before voting deadline; FROZEN: locked after deadline';
...
CREATE TABLE membershipfees.membership_fee_group_rule_snapshot
(
    id                      UUID        NOT NULL PRIMARY KEY,
    membership_fee_group_id UUID        NOT NULL REFERENCES membershipfees.membership_fee_group (id) ON DELETE CASCADE,
    event_type_id           UUID        NOT NULL,
    ranking_short_name      VARCHAR(100) NOT NULL,
    rule_type               VARCHAR(20) NOT NULL,
    rule_percentage         INT         NULL,
...
CREATE INDEX idx_membership_fee_group_rule_snapshot_group ON membershipfees.membership_fee_group_rule_snapshot (membership_fee_group_id);

-- Comments for membership_fee_group_rule_snapshot
COMMENT ON TABLE membershipfees.membership_fee_group_rule_snapshot IS 'Snapshot of co-payment rules copied from the level template at publication time; independent of catalog changes';
COMMENT ON COLUMN membershipfees.membership_fee_group_rule_snapshot.event_type_id IS 'Reference to EventType (no FK — cross-module value object reference)';
COMMENT ON COLUMN membershipfees.membership_fee_group_rule_snapshot.ranking_short_name IS 'Ranking short name as natural key; no FK until ranking table exists (D3)';
...
CREATE TABLE membershipfees.membership_fee_group_members
(
    id                      UUID        NOT NULL PRIMARY KEY,
    membership_fee_group_id UUID        NOT NULL REFERENCES membershipfees.membership_fee_group (id) ON DELETE CASCADE,
    member_id               UUID        NOT NULL,
    ...
    CONSTRAINT uk_membership_fee_group_members UNIQUE (membership_fee_group_id, member_id),
    CONSTRAINT chk_membership_fee_group_members_source CHECK (assignment_source IN ('MEMBER_CHOICE', 'ADMIN_ASSIGNMENT'))
);

-- Indexes for membership_fee_group_members
CREATE INDEX idx_membership_fee_group_members_group ON membershipfees.membership_fee_group_members (membership_fee_group_id);
CREATE INDEX idx_membership_fee_group_members_member ON membershipfees.membership_fee_group_members (member_id);

-- Comments for membership_fee_group_members
COMMENT ON TABLE membershipfees.membership_fee_group_members IS 'Records which members chose (or were assigned to) a fee level for a given year';
COMMENT ON COLUMN membershipfees.membership_fee_group_members.member_id IS 'Reference to Member aggregate (no FK — cross-module value object reference)';
COMMENT ON COLUMN membershipfees.membership_fee_group_members.joined_at IS 'Date the member was added to this fee group';
COMMENT ON COLUMN membershipfees.membership_fee_group_members.assignment_source IS 'MEMBER_CHOICE: member self-selected; ADMIN_ASSIGNMENT: assigned by admin (e.g. emergency assignment after deadline)';
COMMENT ON COLUMN membershipfees.membership_fee_group_members.assigned_by IS 'MemberId of the admin who performed an ADMIN_ASSIGNMENT; null for MEMBER_CHOICE';
...
CREATE TABLE membershipfees.yearly_fee_charge_marker
(
    member_id    UUID      NOT NULL,
...
COMMENT ON TABLE membershipfees.yearly_fee_charge_marker IS 'Idempotency guard: prevents double-posting the yearly membership fee for a (member, year) pair (D6)';
COMMENT ON COLUMN membershipfees.yearly_fee_charge_marker.member_id IS 'Reference to Member aggregate (no FK — cross-module value object reference)';
COMMENT ON COLUMN membershipfees.yearly_fee_charge_marker.charge_year IS 'Calendar year for which the yearly fee was charged';
COMMENT ON COLUMN membershipfees.yearly_fee_charge_marker.charged_at IS 'Timestamp when the fee was successfully posted to finance module';
...
CREATE TABLE events.member_registration_block
(
    member_id  UUID      NOT NULL,
