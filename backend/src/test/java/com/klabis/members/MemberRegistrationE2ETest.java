...
@E2EIntegrationTest
@ActiveProfiles("test")
@DisplayName("Member Registration E2E Test")
class MemberRegistrationE2ETest {
    ...
    void shouldCompleteRegistrationFlowWithOutboxPattern(AssertablePublishedEvents events) throws Exception {
        ...
        // STEP 5: check member is returned in `GET /members`

        // STEP 6: use password setup token to activate member account

        // STEP 7. check activated member can use API (`GET /members`)

        // STEP 8. terminate member

        // STEP 9: check member is NOT returned in `GET /memmbers`

        // STEP 10: check member cannot use API ('GET /members`)
    }
