...
class GetMemberE2ETest {
    ...
    void shouldCreateMemberAndRetrieveById() throws Exception {
        ...
        String memberId = getIdFromLocationHeader(createResult);
    ...
    static final Pattern LOCATION_PATTERN = Pattern.compile("http://localhost/api/members/(.*)");

    static String getIdFromLocationHeader(MvcResult locationHeader) {
        String headerValue = locationHeader.getResponse().getHeader(HttpHeaders.LOCATION);
        Matcher matcher = LOCATION_PATTERN.matcher(headerValue);
        if (matcher.matches()) {
            return matcher.group(1);
        } else {
            Assertions.fail("Unexpected Location header format: got '%s'".formatted(headerValue));
            return "UNREACHABLE";
        }
    }
