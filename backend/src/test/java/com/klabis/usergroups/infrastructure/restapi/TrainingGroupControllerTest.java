...
class TrainingGroupControllerTest {
    ...
    private TrainingGroup buildTrainingGroup(UUID groupUuid, String name, AgeRange ageRange, String trainerUuidStr) {
        ...
        return TrainingGroup.reconstruct(
                new UserGroupId(groupUuid), name, Set.of(trainer), Set.of(), ageRange, null);
    }
    ...
    class GetTrainingGroupTests {
        ...
        @Test
        @DisplayName("should return 200 with full group details including trainers field when user has GROUPS:TRAINING")
        ...
        @Test
        @DisplayName("should return 200 with limited response including trainers field when user lacks GROUPS:TRAINING")
    ...
    class UpdateTrainingGroupTests {
        ...
        void shouldReturn204WhenUpdatingNameOnly() throws Exception {
            ...
            when(trainingGroupManagementService.updateTrainingGroup(any(UserGroupId.class), any()))
                    .thenReturn(group);
        ...
        @Test
        @DisplayName("should return 204 when updating age range")
        @WithKlabisMockUser(memberId = MEMBER_ID, authorities = {Authority.GROUPS_TRAINING})
        void shouldReturn204WhenUpdatingAgeRange() throws Exception {
            TrainingGroup group = buildTrainingGroup(GROUP_UUID, "Juniors", new AgeRange(12, 20), TRAINER_ID);
            when(trainingGroupManagementService.updateTrainingGroup(any(UserGroupId.class), any()))
                    .thenReturn(group);
        ...
        @Test
        @DisplayName("should return 204 when updating trainers")
        @WithKlabisMockUser(memberId = MEMBER_ID, authorities = {Authority.GROUPS_TRAINING})
        void shouldReturn204WhenUpdatingTrainers() throws Exception {
            TrainingGroup group = buildTrainingGroup(GROUP_UUID, "Juniors", new AgeRange(10, 18), TRAINER_ID);
            when(trainingGroupManagementService.updateTrainingGroup(any(UserGroupId.class), any()))
                    .thenReturn(group);
    ...
    class AddTrainingGroupMemberTests {
        ...
        void shouldReturn422WhenDirectMemberAdditionNotAllowed() throws Exception {
            doThrow(new DirectMemberAdditionNotAllowedException())
                    .when(trainingGroupManagementService).addMemberToTrainingGroup(any(UserGroupId.class), any(MemberId.class));
    ...
    class RemoveTrainerTests {
        ...
        void shouldReturn422WhenRemovingLastTrainer() throws Exception {
            MemberId lastTrainer = new MemberId(UUID.fromString(TRAINER_ID));
            doThrow(new UserGroup.CannotRemoveLastOwnerException(lastTrainer))
                    .when(trainingGroupManagementService).removeTrainer(any(UserGroupId.class), any(MemberId.class));
