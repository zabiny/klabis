...
class OrisEventSyncScenarioIntegrationTest {
    ...
    @Nested
    @DisplayName("Self-inflicted dirty marker suppression (openspec sync-skip-self-inflicted-dirty-marker, tasks 4.1-4.3)")
    class SelfInflictedDirtyMarkerSuppression {
        ...
        void applyToLocalDoesNotLeaveRecordDirty() {
            ...
            orisEventSyncAdapter.applyToLocal(eventId.value().toString(), externalProjection);

            Event updated = eventManagementPort.getEvent(eventId, true);
            assertThat(updated.getName()).isEqualTo("Spring Sprint Renamed By ORIS");

            SyncRecord afterApply = synchronizationPort.state(enrolled.getId());
            assertThat(afterApply.getDirtySince())
                    .as("EventsSyncListener must ignore its own SYNCHRONISATION-origin update")
                    .isNull();
        }
        ...
        void manualUpdateLeavesRecordDirty() {
            ...
            SyncRecord afterEdit = synchronizationPort.state(enrolled.getId());
            assertThat(afterEdit.getDirtySince())
                    .as("EventsSyncListener must mark a MANUAL-origin update dirty")
                    .isNotNull();
        }
    }
