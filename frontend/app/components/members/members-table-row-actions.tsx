...
import {Member} from "@/components/members/member-schema";
		...
		<DropdownMenu>
			...
			<DropdownMenuContent align="end" className="w-[160px]">
				...
				<DropdownMenuItem asChild>
					<Link to={`/settings/${row.original.id}/personal`}>
						<Settings className="mr-2 h-4 w-4" /> Nastavení
					</Link>
				</DropdownMenuItem>
				<DropdownMenuItem asChild>
					<Link to={`/members/${row.original.id}/suspend`}>
						<HandCoins className="mr-2 h-4 w-4" /> Finance
					</Link>
				</DropdownMenuItem>
				<DropdownMenuSeparator />
				<DropdownMenuItem asChild>
