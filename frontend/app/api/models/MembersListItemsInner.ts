...
import type { MemberViewCompact } from './MemberViewCompact';
import {
    instanceOfMemberViewCompact,
    MemberViewCompactFromJSON,
    MemberViewCompactFromJSONTyped,
    MemberViewCompactToJSON,
} from './MemberViewCompact';

/**
 * @type MembersListItemsInner
 * 
 * @export
 */
export type MembersListItemsInner = Member | MemberViewCompact;

export function MembersListItemsInnerFromJSON(json: any): MembersListItemsInner {
    return MembersListItemsInnerFromJSONTyped(json, false);
}

export function MembersListItemsInnerFromJSONTyped(json: any, ignoreDiscriminator: boolean): MembersListItemsInner {
    if (json == null) {
        return json;
    }
    if (instanceOfMember(json)) {
        return MemberFromJSONTyped(json, true);
    }
    if (instanceOfMemberViewCompact(json)) {
        return MemberViewCompactFromJSONTyped(json, true);
    }

    return {} as any;
}

export function MembersListItemsInnerToJSON(value?: MembersListItemsInner | null): any {
    if (value == null) {
        return value;
    }

    if (instanceOfMember(value)) {
        return MemberToJSON(value as Member);
    }
    if (instanceOfMemberViewCompact(value)) {
        return MemberViewCompactToJSON(value as MemberViewCompact);
    }
