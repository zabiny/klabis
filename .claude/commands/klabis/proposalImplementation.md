...
allowed-tools: Read(./*), Edit(./*), Glob(./*), Grep(./*), Write(./*), Bash(gradlew:*), Bash(openspec:*)
---
...
Then create TCF in openspec proposal folder. For example for proposal named 'update-member' it would be `./openspec/changes/update-member/{chosen_tcf_name}.md`
...
3. if are there some opened questions or unclear parts, ask user for clarifications here
4. decide how to plan the work
   - if scope of changes is small, let them have implemented by single subagent. 
   - if scope of changes is larger or if affects multiple modules, implement change in multiple iterations. 
   - let every subagent update status of implemented tasks in proposal's task.md after every iteration 
5. after all iterations are completed:
   - run code review and make sure that all findings with at least high priority are fixed.  
   - run all tests and make sure that all tests are compiling and passing
...
- do not describe every changed file and line in TCF file
