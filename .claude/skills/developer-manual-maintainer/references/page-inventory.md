...
- `useHalPageData` hook
- Formuláře — `HalFormButton`, `HalFormsSection`, `HalFormsPageLayout`, `HalFormPanel`, `HalFormDisplay`, `HalFormTemplateButton`, `MultiStepFormModal`
- Modal vs inline formulář (oba ephemeral state v `HalFormContext`, žádný URL param)
- Custom layout přes `children` render-props (`renderInput`, `renderField`, `renderLabel`, `hasField`, `hasType`)
